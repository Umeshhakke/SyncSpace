...
/**
 * Main Whiteboard component with collaborative drawing, code editor, and room management.
 * @param {string} roomId - The current room ID.
 * @param {string} username - The current user's name.
 * @param {string} initialTab - "whiteboard", "editor", or "split".
 */
const Whiteboard = ({ roomId, username, initialTab = "whiteboard" }) => {
  // ---------- Tool state ----------
  const [tool, setTool] = useState("pen");
  const [color, setColor] = useState("#FF6B6B");
  const [brushSize, setBrushSize] = useState(5);
  ...
  // ---------- Yjs delete / add helpers ----------
  const deleteShapeFromYjs = useCallback(
    (shapeId) => {
      if (!shapesArray) return;
      const index = shapesArray.toArray().findIndex((s) => s.id === shapeId);
      if (index !== -1) {
        shapesArray.delete(index, 1);
        console.log(`🗑️ Deleted shape ${shapeId} from Yjs`);
      }
  ...
  const addShapeToYjs = useCallback(
    (shapeData) => {
      if (!shapesArray) return;
      const exists = shapesArray.toArray().some((s) => s.id === shapeData.id);
      if (!exists && shapeData.id) {
        shapesArray.push([shapeData]);
        console.log(`🔄 Re-added shape ${shapeData.id} to Yjs (redo)`);
      }
  ...
  // Publish live stroke preview
  const updateLiveStroke = useCallback(
    (lineData) => {
      if (!awareness) return;
      if (lineData) {
        awareness.setLocalStateField("liveStroke", {
  ...
  // Publish live shape preview
  const updateLiveShape = useCallback(
    (shapeData) => {
      if (!awareness) return;
      if (shapeData) {
        awareness.setLocalStateField("liveShape", {
  ...
  // Publish dragging shape preview
  const updateDraggingShape = useCallback(
    (shapeData) => {
      if (!awareness) return;
      if (shapeData) {
        awareness.setLocalStateField("draggingShape", {
  ...
  useEffect(() => {
    ...
    const handleObserve = (event) => {
      ...
      if (hasDeletion) {
        const remoteIds = new Set(shapesArray.toArray().map((s) => s.id));
        setLines((prev) => prev.filter((item) => !item.id || remoteIds.has(item.id)));
        if (selectedId && !remoteIds.has(selectedId)) {
          setSelectedId(null);
        }
  ...
  useEffect(() => {
    ...
    unsyncedIndices.forEach((idx) => {
      ...
      const uniqueId =
        localItem.id ||
        `shape-${username || "anonymous"}-${Date.now()}-${Math.random()
          .toString(36)
          .substr(2, 9)}`;
      let yjsShape = { id: uniqueId };
  ...
  // ---------- Update shape in Yjs (after drag) ----------
  const updateShapeInYjs = useCallback(
    (shape) => {
      if (!shapesArray) return;
      const arr = shapesArray.toArray();
      const index = arr.findIndex((s) => s.id === shape.id);
  ...
  // ---------- Shape creation helper ----------
  const createShapeObject = (type, startX, startY, endX, endY) => {
    const id = `shape-${username || "anonymous"}-${Date.now()}-${Math.random()
      .toString(36)
      .substr(2, 9)}`;
    return {
      id,
  ...
  const renderDrawing = (item, index) => {
    ...
    switch (type) {
      case "rectangle": {
        ...
        const rectW = Math.abs(width);
        const rectH = Math.abs(height);
        return <Rect key={index} {...shapeProps} x={rectX} y={rectY} width={rectW} height={rectH} />;
      }
      case "circle": {
        const cx = (startX + endX) / 2;
        const cy = (startY + endY) / 2;
        const radius = Math.max(Math.abs(width), Math.abs(height)) / 2;
        return <Circle key={index} {...shapeProps} x={cx} y={cy} radius={radius} />;
      }
      case "triangle": {
        const baseMidX = (startX + endX) / 2;
        const baseMidY = (startY + endY) / 2;
        ...
        const thirdY = baseMidY + perpY * heightThird;
        const points = [startX, startY, endX, endY, thirdX, thirdY, startX, startY];
        return <Line key={index} {...shapeProps} points={points} closed />;
      }
      case "line": {
        return <Line key={index} {...shapeProps} points={[startX, startY, endX, endY]} lineCap="round" />;
      }
      default:
        return null;
    }
  ...
  return (
    ...
    >
      ...
      <div className="whiteboard-main-layout" style={{ flex: 1, position: "relative", display: "flex" }}>
        {/* Whiteboard Canvas */}
        <div
          className="canvas-container"
          style={{
            display: activeTab === "whiteboard" || activeTab === "split" ? "block" : "none",
            flex: activeTab === "split" ? "1 1 50%" : "1 1 100%",
            height: "100%",
            minWidth: 0,
            overflow: "hidden",
            position: "relative",
